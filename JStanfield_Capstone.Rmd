...
boston <- map(urls, read_html) %>% 
  html_node(., "h4")
```

I think a for loop will work to connect the iterator above to the scraping machine blow.

```{r}
### work on acquiring restaurant titles from allmenus (working)
...
allmenu_names <- data.frame( unlist(allmenu_names) )
